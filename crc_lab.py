...
def verify_frame(received_frame: bytes) -> dict:
    ...
    Steps:
      ...
      3. Compare computed FCS against received FCS (or equivalently,
         run CRC over the full frame and check for the magic residue).
    ...
    dict with keys:
        ...
        residue        (int)    – CRC of the entire frame (data + FCS)
    ...
    residue       = crc32(received_frame)
    valid         = (residue == ETHERNET_MAGIC_RESIDUE)
...
def run_tests() -> None:
    ...
    print(f"  Residue        : 0x{result['residue']:08X}")
    ...
    print(f"  Residue        : 0x{result_bad['residue']:08X}")
